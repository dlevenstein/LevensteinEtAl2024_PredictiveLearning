...
# #SBATCH --output=/scratch/dlevenst/sbatch_out/hyperparm_panel_sparse.%A.%a.out  #compute canada
# #SBATCH --job-name=hyperparm_panel_sparse
...
#SBATCH --output=/network/scratch/d/daniel.levenstein/sbatch_out/hyperparm_panel_sparse.%A.%a.out
#SBATCH --job-name=hyperparm_panel_sparse
...
python trainNet.py --savefolder='hyperparm_panel_sparse/' --pRNNtype='AutoencoderPred_LN' --actenc='Onehot' --env='MiniGrid-LRoom-18x18-v0' --seqdur=${TRAINPARMS[1]} --lr=${TRAINPARMS[0]} --bptttrunc=${TRAINPARMS[2]} --hiddensize=${TRAINPARMS[4]} --weight_decay=${TRAINPARMS[3]} --ntimescale=${TRAINPARMS[5]} --dropout=0.15 --noisestd=0.03 --bias_lr=${TRAINPARMS[9]} --sparsity=0.5 --trainBias --numepochs=60 --namext=$ITERATE -s=$SEED --no-saveTrainData

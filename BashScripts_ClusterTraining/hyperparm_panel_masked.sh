...
#SBATCH --output=/scratch/dlevenst/sbatch_out/masked_hyperparm_panel.%A.%a.out  #compute canada
#SBATCH --job-name=masked_hyperparm_panel
...
# #SBATCH --output=/network/scratch/d/daniel.levenstein/sbatch_out/masked_hyperparm_panel.%A.%a.out
# #SBATCH --job-name=masked_hyperparm_panel
...
python trainNet.py --savefolder='hyperparm_panel_masked/' --pRNNtype='thRNN_5win' --actenc='SpeedHD' --seqdur=${TRAINPARMS[1]} --lr=${TRAINPARMS[0]} --bptttrunc=${TRAINPARMS[2]} --hiddensize=${TRAINPARMS[4]} --weight_decay=${TRAINPARMS[3]} --ntimescale=${TRAINPARMS[5]} --dropout=0.15 --noisestd=0.03 --bias_lr=${TRAINPARMS[9]} --sparsity=0.5 --trainBias --numepochs=60 --namext=$ITERATE -s=$SEED --no-saveTrainData  #note: add & to run and not wait for it to finish
